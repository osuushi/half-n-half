...
// Push a node or nodes to the end of a parent
function push(parent, key, nodes) {
  insert(parent, key, parent[key].length, nodes);
}

function siblingsAfter(node) {
  var arr = node._relation.node[node.relation.key];
  var index = node._relation.index;
